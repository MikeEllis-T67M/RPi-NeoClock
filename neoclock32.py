...
while True:
    ...
    suninfo = sun(city.observer, dt.now())

    sunrise = suninfo["sunrise"]
    sunset  = suninfo["sunset"]

    # Convert sunrise and sunset to local times
    sunrise = sunrise + (dt.fromtimestamp(sunrise.timestamp()) - dt.utcfromtimestamp(sunrise.timestamp()))
    sunset  = sunset  + (dt.fromtimestamp(sunset.timestamp())  - dt.utcfromtimestamp(sunset.timestamp()))
